...
object EmployeeAnalyzer extends App {
  ...
  def loadData(filePath: String): List[Employee] = {
  ...
  //Function to check valid record
  def validateRecord(employee: Employee): Boolean = {
    employee.age >= 0
  }
  ...
  def calculateStatistics(employeeList:List[Employee]):Unit={
    ...
    //Education Level:
    val educationLevelDistribution = employeeList.groupBy(_.educationLevel).mapValues(_.size)
  ...
  def analyzeRetentionFactors(employeeList :List[Employee]):Unit={
    ...
    val paymentTierDistribution = employeeList.groupBy(_.paymentTier).mapValues(_.size)
    ...
    val benchStatusDistribution = employeeList.groupBy(_.benchStatus).mapValues(_.size)
    ...
    val experienceDistribution = employeeList.groupBy(_.experienceInCurrentDomain).mapValues(_.size)
  ...
  def segmentEmployees(employeeList: List[Employee]): Unit={
    ...
    val citySegmentation = employeeList.groupBy(_.city).mapValues(_.size)
